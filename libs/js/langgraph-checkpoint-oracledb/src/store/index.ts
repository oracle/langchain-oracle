  ...
  async getDiagnostics(
    ...
    return this.withConnection(async (connection) => {
      ...
      if (vectorMigrations && vectorMigrations.pending.length > 0) {
        issues.push(
          `Vector migrations pending in ${
            this.vectorMigrationTableName
          }: ${vectorMigrations.pending.join(", ")}.`
        );
      }
      if (vectorMigrations && vectorMigrations.future.length > 0) {
        issues.push(
          `Vector migrations recorded in ${
            this.vectorMigrationTableName
          } are newer than this package supports: ${vectorMigrations.future.join(
            ", "
          )}.`
        );
      }

...
import { STORE_MIGRATIONS, VECTOR_MIGRATIONS } from "../store/migrations.js";
...
describe("index configuration validation", () => {
  ...
  test("rejects out-of-range numeric options", () => {
    ...
    for (const [overrides, message] of cases) {
      expect(() => validateOracleIndexConfig(indexConfig(overrides))).toThrow(
        message
      );
    }
  });
...
describe("SQL injection resistance", () => {
  ...
  test.each(injections)("rejects %s", (_label, overrides) => {
    ...
    expect(() =>
      createConfiguredVectorIndexSQL(VECTOR_TABLE, config)
    ).toThrow();
  });
...
DISTANCE COSINE`
  ...
  test("omits the PARAMETERS clause when index_type is absent", () => {
    ...
    expect(
      parametersClause(indexConfig({ index_type: { type: "hnsw" } }))
    ).toBe("PARAMETERS (type HNSW)");
...
describe("store migrations", () => {
  ...
  test("keeps the store versions Python assigned", () => {
    const sql = STORE_MIGRATIONS.map((migration) => migration.sql({ tables }));
  ...
  test("skips the index migration without an index configuration", () => {
    ...
    expect(
      VECTOR_MIGRATIONS[1].condition?.({ tables, index: indexConfig() })
    ).toBe(true);

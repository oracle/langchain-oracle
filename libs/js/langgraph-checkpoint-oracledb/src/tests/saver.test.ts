  ...
  async execute<RowT = Record<string, unknown>>(
    ...
    if (sql.includes("SELECT v")) {
      if (this.options.latestMigrationVersion !== undefined) {
        return {
          rows: [{ V: this.options.latestMigrationVersion } as RowT],
        };
      }
...
describe("OracleCheckpointSaver", () => {
  ...
  test("reads Python native JSON and default MessagePack payloads", async () => {
    ...
    await expect(
      ...
      pendingWrites: [["python-task", "events", { from: "python-write" }]],
    });
  });

  test("stores every versioned channel as a blob and keeps checkpoint JSON value-free", async () => {
    const connection = new FakeConnection();
    const saver = new OracleCheckpointSaver({ connection });
    await saver.setup();
    ...
    expect(storedCheckpoint.channel_values).toEqual({});
    ...
    const blobChannels = (blobBatch?.binds ?? [])
      ...
      .sort();
    expect(blobChannels).toEqual(["bytes", "nested", "primitive", "set"]);
    expect(blobBatch?.options).toHaveProperty("bindDefs");
  });

  test("stores only changed channels and persists deletions as empty blobs", async () => {
    const connection = new FakeConnection();
    const saver = new OracleCheckpointSaver({ connection });
    await saver.setup();
    connection.executions = [];
    ...
    await saver.put(
      { configurable: { thread_id: "thread-delta" } },
      {
        ...emptyCheckpoint(),
        id: "checkpoint-delta",
        channel_values: { changed: "new value", carried: "old value" },
        channel_versions: { changed: 2, carried: 1, removed: 2 },
      },
      { source: "update", step: 1, parents: {} },
      { changed: 2, removed: 2 }
    );
    ...
    expect(storedCheckpoint.channel_values).toEqual({});
    expect(storedCheckpoint.channel_versions).toEqual({
      changed: 2,
      carried: 1,
      removed: 2,
    });
    ...
    const blobRows = blobBatch?.binds ?? [];
    expect(
      blobRows.map((binds) => bindValue<string>(binds.channel)).sort()
    ).toEqual(["changed", "removed"]);
    expect(
      blobRows.find((binds) => bindValue(binds.channel) === "removed")
    ).toMatchObject({
      type: "empty",
      blob: null,
    });
  ...
  test("materializes carried values when appending to a legacy inline checkpoint", async () => {
    const connection = new FakeConnection({
      checkpointRows: [
        {
          CHECKPOINT: {
            ...emptyCheckpoint(),
            channel_values: { carried: "from-python" },
            channel_versions: {
              carried: "00000000000000000000000000000001.0.1",
            },
          },
        },
      ],
    ...
    await saver.put(
      {
        configurable: {
          ...
          checkpoint_id: "python-checkpoint",
        },
      },
      {
        ...emptyCheckpoint(),
        id: "js-checkpoint",
        channel_values: { carried: "from-python", changed: "from-js" },
        channel_versions: {
          carried: "00000000000000000000000000000001.0.1",
          changed: "00000000000000000000000000000002.0.2",
        },
      },
      { source: "update", step: 1, parents: {} },
      { changed: "00000000000000000000000000000002.0.2" }
    ...
    expect(
      (blobBatch?.binds ?? [])
        .map((binds) => bindValue<string>(binds.channel))
        .sort()
    ).toEqual(["carried", "changed"]);
  });

  test("validates the legacy JSON size threshold option", async () => {
    expect(
      () =>

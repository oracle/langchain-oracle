# @oracle/langgraph-checkpoint-oracledb
...
```bash
pnpm add @oracle/langgraph-checkpoint-oracledb \
...
```ts
import { OracleCheckpointSaver } from "@oracle/langgraph-checkpoint-oracledb";
...
Every changed channel value passes through the serializer and is stored in the
versioned `checkpoint_blobs` table. This lets a checkpoint reconstruct its
complete state from `channel_versions` without duplicating unchanged values.
`metadata` is stored as JSON at any size.

| Value                 | Passes through the serializer |
| --------------------- | ----------------------------- |
| Changed channel value | yes                           |
| `metadata`            | no, at any size               |
| Pending writes        | yes                           |

This matters if the serializer is doing more than serialising. An encrypting
serializer applies to every changed channel value, but not to `metadata`,
which remains readable in the database, for instance through
...
import {
  ...
  OracleStore,
} from "@oracle/langgraph-checkpoint-oracledb";
...
```ts
import { OracleStore } from "@oracle/langgraph-checkpoint-oracledb/store";

...
export interface OracleCheckpointSaverOptions {
  ...
  /**
   * @deprecated Channel values are stored as versioned blobs so checkpoints
   * can be reconstructed from `newVersions`. This option is retained and
   * validated for backwards-compatible construction only.
   */
...
export class OracleCheckpointSaver extends BaseCheckpointSaver {
  ...
  constructor(options: OracleCheckpointSaverOptions = {}) {
    ...
    this.tableSuffix = options.tableSuffix
      ...
      : "";
    validateJsonSizeThreshold(options.jsonSizeThresholdMb);
  ...
  async setup(): Promise<void> {
    this.setupPromise ??= this.withTransaction(async (connection) => {
      ...
      if (currentVersion >= migrations.length) {
        throw new Error(
          `Oracle checkpoint schema version ${currentVersion} is newer than the highest supported version ${
            migrations.length - 1
          }.`
        );
      }
  ...
  async put(
    ...
    const blobValues = checkpointValue.channel_values;
    checkpointValue.channel_values = {};
    checkpointValue.channel_versions = {
      ...checkpointValue.channel_versions,
      ...newVersions,
    };
    let blobVersions = newVersions;
    if (
      parentCheckpointId &&
      (await this.parentHasInlineChannelValues(
        threadId,
        checkpointNs,
        parentCheckpointId
      ))
    ) {
      blobVersions = checkpointValue.channel_versions;
    }
...
WHERE (table_name = UPPER(:checkpoints)
  ...
  private async parentHasInlineChannelValues(
    threadId: string,
    checkpointNs: string,
    checkpointId: string
  ): Promise<boolean> {
    const query = buildSelectCheckpointSQL(
      { threadId, checkpointNs, checkpointId, limit: 1 },
      this.tableSuffix
    );
    const [row] = await this.selectCheckpointRows(query.sql, query.binds);
    if (!row) return false;

    const value = rowValue<unknown>(row, "checkpoint");
    const parent = (
      typeof value === "string" ? JSON.parse(value) : value
    ) as Checkpoint;
    return Object.keys(parent.channel_values ?? {}).length > 0;
  }

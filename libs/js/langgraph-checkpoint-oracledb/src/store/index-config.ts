...
export function validateIntegerOption(
  ...
  if (typeof value !== "number" || !Number.isSafeInteger(value)) {
    throw new Error(
      `OracleStore ${label} must be an integer. Received ${describeValue(
        value
      )}.`
    );
  }
...
export function pythonJsonDumps(
  ...
  if (Array.isArray(value)) {
    return `[${value
      .map((item) => pythonJsonDumps(item, options))
      .join(", ")}]`;
  }
  ...
  if (Number(stored.detectedDims) !== config.dims) {
    throw new Error(
      `Dimension mismatch for tableSuffix "${tableSuffix}": existing ${String(
        ...
        config.dims
      }. Different embedding dimensions require different table sets. Use a different tableSuffix or drop existing tables.`
    );
  }

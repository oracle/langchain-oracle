...
async function readStoreMigrationVersions(prefix: string): Promise<number[]> {
  const connection = await oracledb.getConnection(oracleConnection);
  try {
...
async function readVectorMigrationVersions(prefix: string): Promise<number[]> {
  const connection = await oracledb.getConnection(oracleConnection);
  try {
    const result = await connection.execute<{ V: number }>(
...
WHERE prefix = :namespacePath AND key = :key`,
    ...
    return row
      ? {
          ttlMinutes: row.TTL_MINUTES === null ? null : Number(row.TTL_MINUTES),
  ...
  test("supports TTL writes, expiration-aware reads, and manual sweeping", async () => {
    await withStore(
      async (store, tableSuffix) => {
        ...
        await setStoreExpiration(tableSuffix, expiredNamespace, "default", -1);

        await expect(
          store.get(expiredNamespace, "default")
        ).resolves.toBeNull();
...
describeIfOracle("OracleStore suffix isolation", () => {
  test("keeps migrations and data independent across suffixes", async () => {
    ...
    try {
      ...
      await expect(storeB.search(["shared"], { limit: 10 })).resolves.toEqual(
        []
      );
    } finally {
      await storeA.stop();

...
def test_tool_choice_none_after_tool_results() -> None:
    ...
    messages = [
        ...
        AIMessage(
            content="",
            tool_calls=[{"id": "call_1", "name": "get_weather", "args": {"city": "Chicago"}}],
        ),
        ToolMessage(content="Sunny, 65°F", tool_call_id="call_1"),
        AIMessage(
            content="",
            tool_calls=[{"id": "call_2", "name": "get_weather", "args": {"city": "New York"}}],
        ),
        ToolMessage(content="Rainy, 55°F", tool_call_id="call_2"),
        AIMessage(
            content="",
            tool_calls=[{"id": "call_3", "name": "get_weather", "args": {"city": "Seattle"}}],
        ),
